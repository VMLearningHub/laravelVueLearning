<template>
    <div class="flex items-center space-x-2 p-2 bg-slate-200 shadow-sm">
        ...
        <button @click.prevent="tabtype('outside')"
            :class="[
                'cursor-pointer px-3 py-1.5 text-sm rounded-sm transition-colors',
                activeTab === 'outside'
                    ? 'bg-blue-600 text-white font-semibold'
        ...
        <button @click.prevent="tabtype('deactivated')"
            :class="[
                'cursor-pointer px-3 py-1.5 text-sm rounded-sm transition-colors',
        ...
        <button @click.prevent="tabtype('deleted')"
            :class="[
                'cursor-pointer px-3 py-1.5 text-sm rounded-sm transition-colors',
                activeTab === 'deleted'
                    ? 'bg-blue-600 text-white font-semibold'
        ...
        <button @click.prevent="tabtype('highlighted')"
            :class="[
                'cursor-pointer px-3 py-1.5 text-sm rounded-sm transition-colors',
        ...
        <button @click.prevent="tabtype('nominated_by_currently')"
            :class="[
                'cursor-pointer px-3 py-1.5 text-sm rounded-sm transition-colors',
...
onMounted(async() => {
    ...
    const response = await axios.post<CounteUserData>("/users/count-user-data");
    countUserData.value = response.data;
    // console.log(countUserData.value.data.active_user);
